...
def print_ascii_dump(s, width=80):
    # Explicitly remove all whitespace characters
    whitespace_chars = [' ', '\t', '\n', '\r', '\f', '\v']
    s_filtered = ''
    for c in s:
    ...
    # Process in chunks of 'width' characters
    for b in range(0, len(s_filtered), width):
        chunk = s_filtered[b:b+width]
        # Convert non-printable characters to dots
        pdat = ''.join(c for c in chunk if 32 <= ord(c) <= 126)
...
# Example usage:
# print_ascii_dump(your_memory_data)
